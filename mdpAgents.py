...
class MDPAgent(Agent):
    ...
    def __init__(self):
        ...
        self.__WALL = ("WL", 0.0)
        self.__FREE = ("FR", 0.0)
        self.__EDIBLE = ("DE", 400.0)
        self.__GHOST = ("GH", -500.0)
        self.__CONVERGENT = ("CV", None)
        self.__SAFETY_THRESHOLD = -200.0
        self.__THREAT_DECAY_RATE = 100.0
    ...
    @param self: the class itself
    @param state: the current game state
    @param log_mode: a boolean value to indicate whether the log mode is active
    @return None
    """
    def final(self, state, log_mode=False):
        ...
        if log_mode:
            with open("log.csv", "a") as logfile:
                entry = ""
                entry += str(self.__SAFETY_THRESHOLD) + ", "
    ...
    @param threat_decay_rate:
    @return
    """
    def __initialize_utilities(self, targets, target_utility, walls, floors, ghosts, safety_threshold=-100.0, threat_decay_rate=100.0):
        utilities = dict()
        ...
        for ghost in ghosts:
            ...
            utilities[ghost] = self.__GHOST
            queue = util.Queue()
            queue.push(ghost)
            visited = set([ghost])
            while not queue.isEmpty():
                curr_location = queue.pop()
                if utilities[curr_location][1] >= safety_threshold:
                    continue
                ...
                for displacement in displacements:
                    ...
                    if utilities[next_location][1] <= utilities[curr_location][1]:
                        continue
                    utilities[next_location] = (self.__GHOST[0], utilities[curr_location][1] + threat_decay_rate)
    ...
    @param state: the current game state
    @param debug_mode: a boolean value to indicate whether the debug mode is active
    @param ghostbuster_mode: a string to indicate which ghostbuster mode (inactive/defensive/offensive) is in use
    @return None
    """
    def __value_iteration(self, state, debug_mode, ghostbuster_mode="inactive"):
        ...
        self.__utilities = self.__initialize_utilities(self.__foods, self.__FOOD, self.__walls, self.__floors, hostile_ghosts, safety_threshold=self.__SAFETY_THRESHOLD, threat_decay_rate=self.__THREAT_DECAY_RATE)
        early_stopping_point = self.__NORMAL_EARLY_STOPPING_POINT
    ...
    @param self: the class itself
    @param state: the current game state
    @param debug_mode: (optional) a boolean value to indicate whether the debug mode is active
